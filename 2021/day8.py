test1="acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf"
test = """
...
# original puzzle
lines = string.split("\n")
outputValues = list(itertools.chain.from_iterable(list(map(lambda x : x.split(" | ")[1].split(" "), lines))))
ans = len(list(filter(lambda x : len(x) != 5 and len(x) != 6, outputValues)))
# print(ans)
...
for line in lines:
    ...
    s = line.split(" | ")
    output = list(map(lambda x : "".join(sorted(x)), s[1].split(" ")))
    pat = list(map(lambda x : "".join(sorted(x)), s[0].split(" ")))
    pattern = output + pat
    ...
    # 9
    for value in pattern:
        if len(value) == 6 and contains(value, digitDict[4]):
            assert digitDict[9] == "", "blerp"
            digitDict[9] = value
            break
    # 3
    for value in pattern:
        if len(value) == 5 and contains(value, digitDict[7]):
            assert digitDict[3] == "", "blerp"
            digitDict[3] = value
            break
    # 9 if 3
    if digitDict[9] == "" and digitDict[3] != "":
        assert digitDict[9] == "", "blerp"
        digitDict[9] = "".join(sorted(set(digitDict[4] + digitDict[3])))
    # 5
    for value in pattern:
        if len(value) == 5 and contains(digitDict[9], value) and value != digitDict[3]:
            assert digitDict[5] == "", "blerp"
            digitDict[5] = value
            break
    # 9 if 5
    if digitDict[9] == "" and digitDict[5] != "":
        assert digitDict[9] == "", "blerp"
        digitDict[9] = "".join(sorted(set(digitDict[5] + digitDict[1])))
    # 2
    for value in pattern:
        if len(value) == 5 and value != digitDict[5] and value != digitDict[3]:
            assert digitDict[2] == "", "blerp"
            digitDict[2] = value
            break
    # 0
    for value in pattern:
        if len(value) == 6 and value != digitDict[9] and contains(value, digitDict[7]):
            assert digitDict[0] == "", "blerp"
            digitDict[0] = value
            break
    # 6
    for value in pattern:
        if len(value) == 6 and value != digitDict[9] and value != digitDict[0]:
            assert digitDict[6] == "", "blerp"
            digitDict[6] = value
            break
    # 5 if 6
    if digitDict[5] == "" and digitDict[6] != "":
        assert digitDict[5] == "", "blerp"
        c = set(digitDict[8]).difference(set(digitDict[6]))
        digitDict[5] = "".join(sorted(set(digitDict[9]).difference(c)))

    key = dict([(value, str(key)) for key, value in digitDict.items()])
    num = int("".join(list(map(lambda x : key[x], output))))
    print(num)
    total += num
